...
class Sampler:
    ...
    def report_topics(self, vocab, outputfilename, limit=50):
        ...
        """
        topicsfile = open(outputfilename + ".txt", 'w')
    ...
    def sample_probs(self, doc_id, index):
        ...
        for kk in range(self._num_topics):
            ...
            word_topic_prob=self._topics.word_in_topic(kk, term)
            
            n_d_k = self._topics._normalizer[kk]
            alpha= self._alpha[kk]
            Sum_n_d_k= sum(self._topics._normalizer.values())
        
            topic_doc_prob = ((n_d_k * alpha)+0.0)/ (Sum_n_d_k* self._num_topics)
        
            sample_probs[kk]= topic_doc_prob* word_topic_prob
...
if __name__ == "__main__":
    ...
    #search_path = "C:\\Users\\yashu\\\OneDrive\\Desktop\\2nd Sem\\SI 630\\HW4\\hw3-data\\wiki/*.txt"
